...
class PrometheusMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        before_time: float = time.perf_counter()
        status_code: int
        try:
            ...
            after_time: float = time.perf_counter()
...
def setting_otlp(app: ASGIApp, app_name: str, endpoint: str, log_correlation: bool = True) -> None:
    ...
    resource: Resource = Resource.create(attributes={"service.name": app_name})

    # set the tracer provider
    tracer: TracerProvider = TracerProvider(resource=resource)
